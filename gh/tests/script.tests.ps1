Describe 'gh.ps1' -ForEach @(
	...
	BeforeAll {
		...
		$profilePath = Split-Path `
			-Path $profilePath `
			-Parent
		$scriptPath = "${profilePath}\ProfileScripts\gh.ps1"
	}
	...
	It '<Shell>.IsValid' {
		$result = & $Shell `
			-NoProfile `
			-File "${PSScriptRoot}\..\resources\gh.ps1"
		if ($result) {
			Write-Host "${result}"
		}
	...
	Context 'autocompletion' {
		...
		It '<shell>.autocompletion' {
			$block = {
				TabExpansion2 -inputScript 'gh ' -cursorColumn 4 | Select-Object -ExpandProperty CompletionMatches
			}
